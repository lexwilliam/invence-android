...
interface TransactionRepository {
    fun observeTransaction(branchUUID: UUID): Flow<List<Transaction>>
    fun observeSingleTransaction(uuid: UUID): Flow<Transaction?>
    suspend fun upsertTransaction(transaction: Transaction): Either<UpsertTransactionFailure, Transaction>
    suspend fun deleteTransaction(transaction: Transaction): Either<DeleteTransactionFailure, Transaction>
}